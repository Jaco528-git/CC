---
name: baoyu-imagine
description: AI image generation through provider APIs including OpenAI GPT Image 2, Azure OpenAI, Google, OpenRouter, DashScope, Z.AI GLM-Image, MiniMax, Jimeng, Seedream and Replicate. Use when the user explicitly wants provider/model control, reference-image workflows, saved prompt files, batch generation, or repeatable local image outputs. Prefer the system imagegen skill for simple one-off in-chat image creation or editing.
version: 1.58.0
metadata:
  openclaw:
...
`{baseDir}` = this SKILL.md's directory. Main script: `{baseDir}/scripts/main.ts`. Resolve `${BUN_X}` runtime: on Windows PowerShell prefer `$env:APPDATA\npm\bun.cmd` when it exists; otherwise use `bun` if available; otherwise use `npx.cmd -y bun`. On non-Windows, prefer `bun`, then `npx -y bun`. Avoid `npx -y bun` when a local Bun is present because it can trigger network or npm cache failures.
